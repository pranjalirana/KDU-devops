...
function App() {
  ...
  const runtimeUrl = typeof window !== 'undefined' ? window._env_?.REACT_APP_PUBLIC_URL : undefined;
  const react_app_url = (runtimeUrl || process.env.REACT_APP_PUBLIC_URL || process.env.REACT_APP_BASE_URL || 'http://localhost:8080').replace(/\/$/, '');
